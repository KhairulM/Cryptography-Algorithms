...
export function HillCipherDecrypt(cipher, key) {
    ...
    for (let i = 0; i < preparedCipher.length; i += 3) {
        let pair = preparedCipher[i] + preparedCipher[i + 1] + preparedCipher[i + 2];
...
function DecryptPair(pair, inversedKeyMatrix) {
    let multiplicationResult = multiply(inversedKeyMatrix, ConvertTextToNumber(pair));
    let plainText = '';
    for (let i = 0; i < multiplicationResult.size(); i++) {
...
function BuildMatrix(key) {
    ...
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            keyArr[i] = keyArr[i] === undefined ? [key[(i * 3) + j]] : [...keyArr[i], key[(i * 3) + j]];
        }
    }
